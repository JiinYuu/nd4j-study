package com.lmq.ml.nd4j.common;
...
public class Stemmer {
	...
	public Set<String> cutToSetFromFile(File file) {
		Set<String> ret = new HashSet<>();
		if(file.isDirectory()) {
			File[] files = file.listFiles();
			for(File f : files) {
				ret.addAll(cutToSetFromFile(f));
			}
		} else {
			char[] w = new char[501];
	...
	public List<String> cutToListFromFile(File file) {
		List<String> ret = new ArrayList<>();
		if(file.isDirectory()) {
			File[] files = file.listFiles();
			for(File f : files) {
				ret.addAll(cutToListFromFile(f));
			}
		} else {
			char[] w = new char[501];
			try (FileInputStream in = new FileInputStream(file)) {
